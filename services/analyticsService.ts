...
export function buildAlerts(districts: DistrictData[], seismic: SeismicData[]): AlertItem[] {
  ...
  districts.forEach((d) => {
    if (d.air.co2 > 730) {
      alerts.push({
        ...
        districtId: d.id,
        title: "High CO2",
        description: `${d.name} exceeds healthy CO2 baseline.`,
        severity: "high",
    ...
    if (d.trafficLevel > 85) {
      alerts.push({
        ...
        description: `Traffic in ${d.name} is critically high.`,
        severity: "medium",
        timestamp: new Date().toISOString(),
      });
    }
  ...
  seismic.forEach((s) => {
    alerts.push({
      ...
      description: `Intensity ${s.intensity.toFixed(1)} detected near ${s.locationName}.`,
...
export function buildInsights(district: DistrictData): InsightItem[] {
  return [
    {
...
export function buildRecommendations(district: DistrictData): RecommendationItem[] {
  return [
    {
      id: `rec-${district.id}`,
      districtId: district.id,
